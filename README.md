...
App: **https://verakey.mdloglabs.org** (open `/app` on a device with an iCloud Keychain or Google Password
Manager passkey).

| Contract | Address |
|---|---|
| `VeraKeyFactory` (Stylus) | [`0x6217e8372aa97b1d48730347b44a636c5878f1dd`](https://sepolia.arbiscan.io/address/0x6217e8372aa97b1d48730347b44a636c5878f1dd) |
| `VeraKeyAccount` implementation (Stylus) | [`0x88abfff2cdc7a5ffd6990204dc4ee3fffe34cc9d`](https://sepolia.arbiscan.io/address/0x88abfff2cdc7a5ffd6990204dc4ee3fffe34cc9d) |
| `HonkVerifier` (Solidity, generated by bb, optimized ZK) | [`0x9d79c603a39df96b2bE254719B99e4592094678f`](https://sepolia.arbiscan.io/address/0x9d79c603a39df96b2bE254719B99e4592094678f) |
...
Accounts are EIP-1167 clones created by the factory; the full configuration is in
...
3. **Proof.** The circuit (`circuits/webauthn`, 56,528 gates) proves the signature, the rpId hash and the UP/UV flags under a hidden key, and derives the owner nullifier from the key, the passkey's **PRF secret** and the app id. Without the PRF secret, which never leaves the authenticator, even a leaked public key cannot link a user's accounts.
...
| UltraHonk proof of a P-256 passkey signature, generated in the browser | real, ~1.9 s with 8 threads (Chrome, measured) |
...
| `pay` (proof + policy + two USDG transfers) | 1,073,882 gas (4,171,302 before), of which the Stylus account logic is ~195k; ≈ $0.06 on Arbitrum One at 0.02 gwei and ETH $2,668 |
| `createAccount` (EIP-1167 clone + storage init) | ~479,000 gas |
...
Foundry, Docker, nargo `1.0.0-beta.25`, bb `5.2.0` (the pair Aztec 5.2.0 ships).
